...
public class Player : MonoBehaviour
{
    ...
    [Range(1, 100)] public float dashSpeed;
    ...
    [Range(.01f, 2)] public float decelSpeed;
    ...
    [Header("GRAB & PICKUP")]
    [SerializeField] private LayerMask canGrabLayer;
    [SerializeField] private int raysNumber;
    ...
    private void DefineSpeeds()
    {
        ...
        isShifting = crouch || (hasSmthAboveHead && bc.size == new Vector2(9, 7.5f));
    ...
    private void Crouch()
    {
        ...
        { 
            ...
            _jumpForce = jumpForce / crouchJump * multiplier;
        }
        else
        {
            bc.size = new Vector2(9, 15);
            bc.offset = new Vector2(0, 0);
            _jumpForce = jumpForce * multiplier;
        }
    ...
    private void CheckVelocities()
    {
        if (isGrounded && !isShifting) 
            _maxSpeed = maxSpeed * multiplier;
        if (!isGrounded && !isShifting) 
            _maxSpeed = maxSpeed / jumpSpeed * multiplier;
        if (isGrounded && isShifting) 
            _maxSpeed = maxSpeed / crouchSpeed * multiplier;
        if (!isGrounded && isShifting) 
            _maxSpeed = maxSpeed / crouchSpeed / jumpSpeed * multiplier;
    }
    ...
    {
        List<RaycastHit2D> canGrabHits = new List<RaycastHit2D>();
        float deltaAngle = 0;
        for (int i = 0; i < raysNumber; i++)
        {
            deltaAngle = i * (360 / (float)raysNumber);
            float radAngle = deltaAngle * Mathf.Deg2Rad;
            ...
            dir.Normalize();
            RaycastHit2D hit = Physics2D.Raycast(transform.position, dir, canGrabRadius, canGrabLayer);
            if (hit.collider != null)
                canGrabHits.Add(Physics2D.Raycast(transform.position, dir, canGrabRadius, canGrabLayer));
        }
        ...
        for (int i = 0; i < canGrabHits.Count; i++)
        {
        ...
        Rigidbody2D[] canGrabObjects = canGrabHits.Select(i => i.collider.GetComponentInParent<Rigidbody2D>()).ToArray();
